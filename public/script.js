    ...
    makeOrUpdate('h-staffChart', {
        ...
        data: {
            ...
            datasets: [{
                ...
                data: ratioData,
                backgroundColor: ratioData.map(v => v > 9 ? '#F76B4F' : v > 6 ? '#FBBF24' : '#A78BF5'),
                borderRadius: 5
            }]
        },
        options: {
...
function renderServicePageWithData(inputData) {
    ...
    if (insightEl) insightEl.innerHTML =
        `Rata-rata kepuasan pasien berada di angka <strong style="color:#fff">${avgSat}/5</strong> : ${satStatus}. Poli dengan skor tertinggi adalah <strong style="color:#fff">${bestPoli}</strong>. ${waitStatus} Total pasien baru tercatat <strong style="color:#fff">${totalNewPat}</strong> kunjungan. ${rekomendasi}`;
}
...
function renderFinancePage() {
    ...
    if (finInsight) {
        ...
        const targetStatus = underTarget.length === 0
            ...
            ? `Semua unit layanan <strong style="color:#F76B4F">belum mencapai target</strong> sehingga perlu evaluasi menyeluruh.`
        ...
        finInsight.innerHTML =
            `Total pendapatan periode ini <strong style="color:#fff">Rp ${Math.round(totalRev)} Juta</strong>.${trenKalimat} Kontributor terbesar: <strong style="color:#fff">${topUnit.label}</strong> (Rp ${Math.round(topUnit.val)} Juta), terendah: <strong style="color:#fff">${botUnit.label}</strong> (Rp ${Math.round(botUnit.val)} Juta). ${targetStatus}`;
    }
}
...
function renderHRPageWithData(inputData) {
    ...
    makeOrUpdate('hr-staffPatientChart', {
        ...
        data: {
            ...
            datasets: [{
                label: 'Pasien per Dokter',
                data: ratioData,
                backgroundColor: ratioData.map(v => v > 9 ? '#F76B4F' : v > 6 ? '#FBBF24' : '#34C98F'),
                borderRadius: 5
            }]
        },
        options: {
    ...
    if (hrTbody) {
        ...
        hrTbody.innerHTML = HR_POLI.map((poli, i) => {
            ...
            const ratioColor = ratio > 9 ? 'color:#DC2626;font-weight:600'
                             : ratio > 6  ? 'color:#D97706;font-weight:600'
                             : 'color:#059669';
            return `<tr>
                <td>${poli}</td>
                <td>${dok}</td>
    ...
    if (hrInsight) {
        ...
        const overloadedPoli = HR_POLI.filter((p, i) => ratioData[i] > 9);
        const warnPoli       = HR_POLI.filter((p, i) => ratioData[i] > 6 && ratioData[i] <= 9);
        const normalPoli     = HR_POLI.filter((p, i) => ratioData[i] <= 6);

        // Status beban kerja keseluruhan
        const bebanStatus = parseFloat(avgRatio) > 9
        ...
        if (overloadedPoli.length > 0)
            detailBeban += ` <strong style="color:#F76B4F">${overloadedPoli.join(', ')}</strong> dalam kondisi kelebihan beban (&gt;9 pasien/dokter).`;
...
function renderEfficiencyPage() {
    ...
    if (effInsight) {
        ...
        const alosStatus = avgALOS <= 4
            ...
            : avgALOS <= 5
            ? `<strong style="color:#FBBF24">sedikit di atas target (${avgALOS} hari)</strong> sehingga perlu optimalisasi protokol pemulangan`
            : `<strong style="color:#F76B4F">melebihi target (${avgALOS} hari)</strong> sehingga perlu audit klinis`;
        ...
        if (highALOS.length > 0 && goodALOS.length > 0)
            ...
            alosDetail = ` Seluruh bangsal melebihi target ALOS sehingga perlu evaluasi menyeluruh.`;
        else
            alosDetail = ` Seluruh bangsal berhasil memenuhi target ALOS &lt;4 hari.`;

        effInsight.innerHTML =
            `Rata-rata BOR saat ini ${borStatus}.${borDetail} ALOS rata-rata ${alosStatus}.${alosDetail}`;
    }
}
    ...
    if (raInsight) {
        ...
        if (wardFilter === 'all') {
            wardKalimat = overTargetWards.length === 0
                ...
                ? ` Semua bangsal melebihi target sehingga perlu evaluasi protokol di seluruh unit.`
        ...
        raInsight.innerHTML =
            `Rata-rata readmission rate periode ini ${rateStatus}.${trenKalimat}${wardKalimat}`;
    }
}
